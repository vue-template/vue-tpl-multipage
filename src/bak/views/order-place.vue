  ...
  <Form class="order-place" ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="80">
    <section class="info-panel">
      <Form-item label="出发地" prop="onSitePoi">
        <Row>
          <Col span="5">
            <Select v-model="formValidate.onSiteCity" placeholder="城市" not-found-text="无搜索结果" filterable @on-change="formValidate.onSitePoi = ''">
          ...
          <Col span="1">&nbsp;</Col>
          <Col span="18">
            <Select v-model="formValidate.onSitePoi" placeholder="请填写出发地" not-found-text="无搜索结果" filterable remote :remote-method="remoteOnSite" :loading="loading">
      ...
      <Form-item label="目的地" prop="offSitePoi">
        <Row>
          <Col span="5">
            <Select v-model="formValidate.offSiteCity" placeholder="城市" not-found-text="无搜索结果" filterable remote :remote-method="remoteOffCity" :loading="loading" @on-change="formValidate.offSitePoi = ''">
          ...
          <Col span="1">&nbsp;</Col>
          <Col span="18">
            <Select v-model="formValidate.offSitePoi" placeholder="请填写目的地" not-found-text="无搜索结果" filterable remote :remote-method="remoteOffSite" :loading="loading">
    ...
    <header class="type-tab">
      <ul>
        <li :class="{'active': formValidate.currentType==1}" @click="switchType(1)">单程</li>
        <li :class="{'active': formValidate.currentType==2}" @click="switchType(2)">往返</li>
        <li :class="{'active': formValidate.currentType==3}" @click="switchType(3)">包车</li>
      </ul>
    </header>
        ...
        <Row>
          <Col span="11">
            <Form-item prop="onDate">
              <Date-picker type="date" :options="dateOption" placeholder="请选择出发日期" v-model="formValidate.onDate" :editable="false"></Date-picker>
            </Form-item>
          </Col>
          <Col span="1">&nbsp;</Col>
          <Col span="12">
            <Form-item prop="onTime">
              <Time-picker type="time" format="HH:mm" placeholder="请选择出发时间" v-model="formValidate.onTime" :editable="false"></Time-picker>
            </Form-item>
          </Col>
        </Row>
      </Form-item>
          ...
          <Row>
            <Col span="11">
              <Form-item prop="offDate">
                <Date-picker type="date" :options="dateOption" placeholder="请选择返程日期" v-model="formValidate.offDate" :editable="false"></Date-picker>
              </Form-item>
            </Col>
            <Col span="1">&nbsp;</Col>
            <Col span="12">
              <Form-item prop="offTime">
                <Time-picker type="time" format="HH:mm" placeholder="请选择返程时间" v-model="formValidate.offTime" :editable="false"></Time-picker>
              </Form-item>
            </Col>
          </Row>
        </Form-item>
...
<script>
  ...
  // 定义已开通城市
  const validCities = ['028', '010', '0755', '020', '021', '029', '027', '023']
  ...
  export default {
    data () {
      return {
        ...
        formValidate: {
          ...
          onCityList: () => {
            return CITYS.filter(
              item => validCities.join(',').indexOf(item.code) >= 0
            ).map(item => {
    ...
    created () {
      this.geoLocation(result => {
        const cityCode = result.addressComponent.citycode
        if (cityCode) {
          this.formValidate.onSiteCity = validCities.find(item => cityCode.toString() === item.toString()) || ''
        }
      })
    },
