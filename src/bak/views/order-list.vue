<template>
  <div class="order-list">
    <div class="w-filter">
      ...
      <div class="item">
        <Button class="button" type="primary" @click="filterOrder(filter)">搜索</Button>
      </div>
    </div>
      ...
      <tbody>
        <template v-for="order in orderList">
          <tr>
            ...
            <td>￥{{order.amount | fen2yuan}}</td>
            <td>
              <div class="actions">
                <a @click="viewOrder(order.order_id)">查看订单</a>
                <a v-if="order.display_status < 5" @click="cancelOrder(order)">取消订单</a>
                <a v-if="order.display_status == 2" @click="payOrder(order)">立即支付</a>
                <countdown v-if="order.display_status == 1" :start-time="order.create_time"></countdown>
              </div>
            </td>
          </tr>
          <tr v-if="order.display_status == 1">
            <td colspan="9">
              <div class="offerbox">
                <div v-if="offerLength" class="header">
                  <h3>已报价车企</h3>
                  <router-link :to="{name: 'offer-list', query: {id: order.order_id}}">更多报价</router-link>
                </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      ...mapGetters(['curOrderInfo', 'offerLength'])
    },
    activated() {
      this.init()
      // console.log('test')
